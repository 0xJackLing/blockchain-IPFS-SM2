...
def reset():
    ...
    tx_hash = mycontract.functions.reset_count().transact({'from': web3.eth.accounts[0],'gas': 4200000, 'gasPrice': 210000})
    tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
    time.sleep(5)
def sentInfo():
    #hash上链完成并触发合约中的事件，以便备份服务器监听
    tx_hash = mycontract.functions.sentInfo().transact({'from': web3.eth.accounts[0],'gas': 420000, 'gasPrice': 21000})
    tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)

def onchain(_hash,_name,_size):
    #将得到hash后的文件信息上链
    tx_hash = mycontract.functions.addFile(_name,_hash,_size).transact({'from': web3.eth.accounts[0],'gas': 420000, 'gasPrice': 21000})
    tx_receipt = web3.eth.wait_for_transaction_receipt(tx_hash)
    time.sleep(5)
